...
const initRequest = async () => {
  ...
  try {
    const [categoryRes] = await Promise.all([
      proxy.$axios.get('contact_category/', {params: search_param}),
    ])
...
const handleDelete = (index, row) => {
  proxy.$axios
      .delete(`contact_category/${row.id}/`)
      .then(() => {
  ...
  proxy.$refs.submitRef.validate((valid) => {
    if (valid) {
      ...
      if (currentItem.id === null) {
        // 添加数据
        proxy.$axios.post("contact_category/", formData).then((res) => {
        ...
        // 更新数据
        proxy.$axios.patch(`contact_category/${currentItem.id}/`, formData).then((res) => {
